...
export default async function PublicCasePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: updates } = await supabase.from('case_updates').select('id, body, created_at').eq('case_id', id).order('created_at', { ascending: false })

  const documents = docs ?? []
  const photos = documents.filter((x) => x.doc_type === 'photo')
  const otherDocs = documents.filter((x) => x.doc_type !== 'photo')
        ...
        <div className="pc-grid" style={{ marginTop: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {updates.map((u) => (
                    <div key={u.id} style={{ borderLeft: '3px solid var(--ind)', paddingLeft: 14 }}>
                      ...
                      <div style={{ fontSize: 14.5, lineHeight: 1.7, whiteSpace: 'pre-wrap', marginTop: 2 }}>{u.body}</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          <aside>
            <div className="pc-side" style={{ border: '1px solid var(--line)', borderRadius: 18, padding: 22, boxShadow: '0 6px 24px rgba(30,30,50,.06)' }}>
              ...
              <div style={{ fontSize: 32, fontWeight: 800, letterSpacing: '-1px', margin: '2px 0 14px' }}>{Number(c.requested_amount).toLocaleString()} <span style={{ fontSize: 16, color: 'var(--muted)' }}>{cp.baht}</span></div>
              <div style={{ height: 10, background: '#f0f1f4', borderRadius: 999, overflow: 'hidden' }}><div style={{ width: `${pct}%`, height: '100%', background: 'var(--ind)', borderRadius: 999 }} /></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, fontSize: 13 }}>
                <span style={{ color: 'var(--ind-d)', fontWeight: 700 }}>{cp.progress} {pct}%</span>
                <span style={{ color: tag.fg, fontWeight: 700 }}>{tag.label}</span>
              </div>
              <Link href="/dashboard/donate" className="pc-btn" style={{ display: 'block', textAlign: 'center', marginTop: 18, padding: '14px', borderRadius: 12, fontWeight: 800, fontSize: 16 }}>{cp.donate}</Link>
